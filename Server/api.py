...
from Server.gemini_model import analyze_message
from ML_model.ml_model import predict_resolution_time
...
def _reply_with_eta(ticket, message: str) -> str:
    try:
        eta = predict_resolution_time(ticket)
        return f"{message} Estimated resolution time is {eta} minutes."
    except Exception:
        return message
...
async def telegram_webhook(request: Request) -> Dict[str, Any]:
    ...
    if ingress:
        analysis = analyze_message(ingress.message)
        if analysis.is_complaint:
            ingress.error_type = analysis.complaint_type
            ticket = persist_ingress(app.state.session_factory, ingress)
            await send_telegram_message(ticket.userid, _reply_with_eta(ticket, analysis.message))
        else:
            await send_telegram_message(ingress.userid, analysis.message)
...
async def discord_webhook(request: Request) -> Dict[str, Any]:
    ...
    if userid and message:
        ...
        if analysis.is_complaint:
            ingress.error_type = analysis.complaint_type
            ticket = persist_ingress(app.state.session_factory, ingress)
            await send_discord_dm(ticket.userid, _reply_with_eta(ticket, analysis.message))
        else:
            await send_discord_dm(userid, analysis.message)
...
async def slack_webhook(request: Request) -> Dict[str, Any]:
    ...
    if ingress:
        ...
        if analysis.is_complaint:
            ingress.error_type = analysis.complaint_type
            ticket = persist_ingress(app.state.session_factory, ingress)
            await send_slack_message(ticket.userid, _reply_with_eta(ticket, analysis.message))
        else:
           await send_slack_message(ingress.userid, analysis.message)
...
def list_tickets(db: Session = Depends(get_session)) -> List[Dict[str, Any]]:
    """Returns all tickets with basic metadata and the latest message."""
    tickets = list(
        db.execute(
            select(Ticket).options(joinedload(Ticket.messages))
        ).unique().scalars().all()
    )
    results = []
    for ticket in tickets:
        # Find the latest message by time
        latest_msg = None
        if ticket.messages:
            sorted_msgs = sorted(ticket.messages, key=lambda m: m.time, reverse=True)
            latest_msg = sorted_msgs[0].message if sorted_msgs else None
        results.append({
...
def get_stats(db: Session = Depends(get_session)) -> Dict[str, Any]:
    ...
    # Channel breakdown
    telegram_count = db.execute(select(func.count(Ticket.ticket_id)).where(Ticket.channel == "telegram")).scalar()
    discord_count = db.execute(select(func.count(Ticket.ticket_id)).where(Ticket.channel == "discord")).scalar()
    slack_count = db.execute(select(func.count(Ticket.ticket_id)).where(Ticket.channel == "slack")).scalar()

    # Urgency breakdown
    high_count = db.execute(select(func.count(Ticket.ticket_id)).where(Ticket.urgency == "high")).scalar()
    medium_count = db.execute(select(func.count(Ticket.ticket_id)).where(Ticket.urgency == "medium")).scalar()
    low_count = db.execute(select(func.count(Ticket.ticket_id)).where(Ticket.urgency == "low")).scalar()
    ...
    return {
        ...
        "telegram_tickets": telegram_count,
        "discord_tickets": discord_count,
        "slack_tickets": slack_count,
        "high_urgency_tickets": high_count,
        "medium_urgency_tickets": medium_count,
        "low_urgency_tickets": low_count,
    }
