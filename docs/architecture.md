...
### A. NODE 1: External API permissions
  ...
  3. convert all the data from telegram and discord bots to a schema so that further it can be added to database with ease. Schema will include, ticket_id(auto-generated), channel(telegram or discord), userid(of user on channel),username(on channel), time(time at which ticket was generated), status(assigned/processing/closed, (assigned as default at beginning)).
