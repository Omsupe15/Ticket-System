...
from Server.gemini_model import analyze_user_mood  # Analyze mood using Gemini model
...
def create_or_append_ticket(session: Session, ingress: TicketIngress) -> Ticket:
    ...
    mood = analyze_user_mood(ingress.message)  # Analyze mood using Gemini model
    ingress.set_urgency_based_on_mood(mood)  # Set urgency based on mood
    ...
    if latest is None or latest.status == "closed":
        ticket = Ticket(
            ...
            status="assigned",
            channel=ingress.channel.value,
            error_type=ingress.error_type,
            urgency=ingress.urgency,
            time_updated=ingress.time_updated,
            time_closed=ingress.time_closed,
    ...
    latest.status = "processing"  # Update status and timestamps
    session.commit()
